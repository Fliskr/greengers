...
import { getCheckoutOrders } from '../../actions/actionCreator';
...
class CheckOut extends Component {
  ...
  async handleCheckInButton(e) {
    ...
    delete state.error;
    await request(`api/orders/${state.id}/operations/checkOut`, { method: "POST", body: JSON.stringify(state) },'text');
    this.clearState();
  }
  ...
  async handleDeleteButton(e){
    e.preventDefault();
    await request(`api/orders/${this.state.id}`,{method:"DELETE"},'json')
    this.clearState();
  }
    ...
    return (
      <form className="form">
       <div className="row">
        ...
        <div className="row">
          <button type="submit" disabled={this.state.id==="" || this.state.status==="Failed inspection"} onClick={this.handleCheckInButton.bind(this)}>{getLocaleText("checkOut")}</button>
          <button type="submit" onClick={this.handleCancelButton.bind(this)}>{getLocaleText("cancel")}</button>
          <button type="submit" disabled={this.state.id==="" || this.state.status!=="Failed inspection"} onClick={this.handleDeleteButton.bind(this)}>{getLocaleText("delete")}</button>
        </div>
     </form>
    )
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    getCheckoutOrders: () => dispatch(getCheckoutOrders())
  }
}
